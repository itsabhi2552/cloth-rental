...
function BlockedUsers() {
  ...
  function unblockUser(i){
    let token=localStorage.getItem("token");
    fetch("http://localhost:3000/unblock_user", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({token:token,id:AllUser[i].user_id}),
    }).then((res)=>{
        if(res.status===200){
            setAllUser(
                AllUser.filter((item,index)=>{
                    return index!==i;
                })
            )
        }
    ...
    useEffect(()=>{
        let token=localStorage.getItem("token");
        fetch("http://localhost:3000/blocked_user", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    <div className='row '>
      ...
      <div className='col'>
            <div className="px-3">
                ...
                <span className="text-white fs-4">Blocked Users</span>
            </div>
                            ...
                            return(
                                <>
                                    <tr key={index}>
                                        ...
                                        <td>{item.address}</td>
                                        <td><button className="btn btn-danger" onClick={()=>unblockUser(index)}>unblock</button></td>
                                    </tr>
                                </>
                            )
                        })
