...
function BlockedDeliveryBoys() {
  const [toggle, setToggle] = useState(true)    
  const Toggle = () => {setToggle(!toggle)}  
  const [AllDeliveryBoys,setAllDeliveryBoys]=useState([]);
  
  function unblockDeliveryBoys(i){
    let token=localStorage.getItem("token");
    fetch("http://localhost:3000/unblock_delivery_boy", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({token:token,id:AllDeliveryBoys[i].user_id}),
    }).then((res)=>{
        if(res.status===200){
            setAllDeliveryBoys(
                AllDeliveryBoys.filter((item,index)=>{
                    return index!==i;
                })
            )
        }
    ...
    useEffect(()=>{
        let token=localStorage.getItem("token");
        fetch("http://localhost:3000/blocked_delivery_boys", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    <div className='row '>
      ...
      <div className='col'>
            <div className="px-3">
                ...
                <span className="text-white fs-4">Blocked Delivery Boys</span>
            </div>
            ...
            <table class="table caption-top bg-white rounded mt-2" >
                ...
                <tbody>
                    {
                        AllDeliveryBoys.map((item,index)=>{
                            return(
                                <>
                                    <tr key={index}>
                                        ...
                                        <td>{item.address}</td>
                                        <td><button className="btn btn-danger" onClick={()=>unblockDeliveryBoys(index)}>unblock</button></td>
                                    </tr>
                                </>
                            )
                        })
...
export default BlockedDeliveryBoys;
