...
function ActiveDeliveryBoys() {
  const [toggle, setToggle] = useState(true)    
  const Toggle = () => {setToggle(!toggle)}  
  const [AllDeliveryBoys,setAllDeliveryBoys]=useState([]);
  function blockDeliveryBoys(i){
    let token=localStorage.getItem("token");
    fetch("http://localhost:3000/block_delivery_boy", {
    method: "POST",
    ...
    useEffect(()=>{
        let token=localStorage.getItem("token");
        fetch("http://localhost:3000/active_delivery_boys", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    <div className='row '>
      ...
      <div className='col'>
            <div className="px-3">
                ...
                <span className="text-white fs-4">Active Delivery Boys</span>
            </div>
                            ...
                            return(
                                <>
                                    <tr key={index}>
                                        ...
                                        <td>{item.address}</td>
                                        <td><button className="btn btn-danger" onClick={()=>blockDeliveryBoys(index)}>Block</button></td>
                                    </tr>
                                </>
                            )
                        })
