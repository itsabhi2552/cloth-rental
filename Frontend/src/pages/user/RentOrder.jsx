...
function RentOrder() {
  ...
  function Return(i){
    ...
    body: JSON.stringify({token:token,order_id:AllProduct[i].order_id}),
        ...
        if(res.status===200){
            setAllProduct([...AllProduct], AllProduct[i].order_status = "request to return");;
        }
    })
}
    useEffect(()=>{
        let token=localStorage.getItem("token");
        fetch("http://localhost:3000/get_user_order_details_for_rent", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
                                ...
                                return(
                                    <>
                                        ...
                                        /></td>
                                            ...
                                            <td>{item.cloth_returning_date.split('T')[0]}</td>
