...
function OnTheWayOrders() {
  ...
  function Delivered(i){
    ...
    fetch("http://localhost:3000/delivered_order", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({token:token,order_id:Orders[i].order_number}),
    }).then((res)=>{
        if(res.status===200){
            setOrders(
    ...
    useEffect(()=>{
        let token=localStorage.getItem("token");
        fetch("http://localhost:3000/get_on_the_way_orders", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    <div className='row '>
      ...
      <div className='col'>
            <div className="px-3">
                ...
                <span className="text-white fs-4">All On The Way Orders</span>
            </div>
                            ...
                            return(
                                <>
                                    <tr key={index}>
                                        ...
                                        <td><img className="mt-2" src={`http://localhost:3000/${item.product_image}`} style={{height: "200px",width: "200px"}}/></td>
                                        <td>{item.order_number}</td>
                                        <td>{item.product_name}</td>
                                        <td>{item.cust_name}</td>
                                        <td>{item.cust_mobile_number}</td>
                                        <td>{item.pickup_add}</td>
                                        <td>{item.delivery_add}</td>
                                        <td><button className="btn btn-danger" onClick={()=>Delivered(index)}>Delivered</button></td>
                                    </tr>
                                </>
                            )
                        })
